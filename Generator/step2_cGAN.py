...
def main():
    data_name = 'fillseq_20240614132835_tiqu'
    seqL = 165
    ...
    train_data, test_data = DataLoader(LoadData(is_train=True, path='../Generator/cachetest1/{}.csv'.format(data_name), split_r=1),
                                       batch_size=32, shuffle=True), DataLoader(LoadData(is_train=True, path='../Generator/cachetest1/{}.csv'.format(data_name), split_r=1), batch_size=32)
    train_data = get_infinite_batches(train_data)
    logger = utils.get_logger(log_path='cachetest2/training_log/', name=data_name)
    n_critics = 5
    n_iters = 5000
    ...
    for i in range(n_iters):
        ...
        if i % 100 == 99:
            ...
            d_loss, g_loss, g_l1 = 0, 0, 0
            csv_name = save_sequence(tensorSeq, tensorInput, tensorRealB, save_path='cachetest2/', name='inducible_{}_'.format(data_name), cut_r=1)
            utils.kmer_frequency(csv_name, '../Generator/cachetest1/{}.csv'.format(data_name), k=4, save_path='cachetest2/figure/', save_name=data_name + str(i))
            csv2fasta(csv_name, 'cachetest2/gen_', 'iter_{}'.format(i))
